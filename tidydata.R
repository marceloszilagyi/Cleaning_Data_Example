...
# 1st Step: get the data from site; get a list of files
###########################################################

 # set a new directory to place the data (FUCI100), to avoid conflict with other files/directories
  # if the directory does not exist, create the directory 
  work.dir <- paste0 ("~/","FUCI100")
  ifelse(file_test("-d", "FUCI100"),"directory exists",dir.create(work.dir))
    # setup the work directory to be the new directory
  setwd(work.dir)

  # download the zip file in temp dirdirectory; timeout extended to avoid issues during download
  temp<-tempfile()
  options(timeout = 400)
  url<-"https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
  download.file(url,temp) 
  
  # unzip the file to the work directory
  unzip(temp)
  # set the working directory to be the the 'UCI HAR Dataset'
